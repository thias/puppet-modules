...
MYSQL_USER_PRIVS_4 = [ :select_priv, :insert_priv, :update_priv, :delete_priv,
...
MYSQL_USER_PRIVS_50 = [ :select_priv, :insert_priv, :update_priv, :delete_priv,
	:create_priv, :drop_priv, :reload_priv, :shutdown_priv, :process_priv,
	:file_priv, :grant_priv, :references_priv, :index_priv, :alter_priv,
	:show_db_priv, :super_priv, :create_tmp_table_priv, :lock_tables_priv,
...
MYSQL_USER_PRIVS_51 = MYSQL_USER_PRIVS_50 + [ :event_priv, :trigger_priv ]

MYSQL_DB_PRIVS_4 = [ :select_priv, :insert_priv, :update_priv, :delete_priv,
	:create_priv, :drop_priv, :grant_priv, :references_priv, :index_priv,
	:alter_priv, :create_tmp_table_priv, :lock_tables_priv
]

MYSQL_DB_PRIVS_50 = [ :select_priv, :insert_priv, :update_priv, :delete_priv,
	:create_priv, :drop_priv, :grant_priv, :references_priv, :index_priv,
	:alter_priv, :create_tmp_table_priv, :lock_tables_priv, :create_view_priv,
	:show_view_priv, :create_routine_priv, :alter_routine_priv, :execute_priv
]

MYSQL_DB_PRIVS_51 = MYSQL_DB_PRIVS_50 + [ :event_priv, :trigger_priv  ]
...
Puppet::Type.type(:mysql_grant).provide(:mysql) do
	...
	def all_privs_set?
		version = mysql "mysql", "-BNe", 'select version()'
		all_privs = case split_name(@resource[:name])[:type]
			when :user
				if version[0,3] == "4.0"
					MYSQL_USER_PRIVS_4
				elsif version[0,3] == "5.0"
					MYSQL_USER_PRIVS_50
				else
					MYSQL_USER_PRIVS_51
				end
	...
	def privileges=(privs)
		...
		when :user
			...
			if version[0,3] == "4.0"
				all_privs = MYSQL_USER_PRIVS_4
			elsif version[0,3] == "5.0"
				all_privs = MYSQL_USER_PRIVS_50
			else
				all_privs = MYSQL_USER_PRIVS_51
			end
		when :db
			stmt = 'update db set '
			where = ' where user="%s" and host="%s"' % [ name[:user], name[:host] ]
			if version[0,3] == "4.0"
				all_privs = MYSQL_DB_PRIVS_4
			elsif version[0,3] == "5.0"
				all_privs = MYSQL_DB_PRIVS_50
			else
				all_privs = MYSQL_DB_PRIVS_51
			end
		end
